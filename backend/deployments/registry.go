...
// LoadEmbeddedWithManifest loads the embedded reviewed schemas and dependency
// records together with one explicitly supplied deployment manifest. This is
// used by isolated local E2E runs: the manifest is generated by the
// authoritative Foundry deployment and is never committed or treated as a
// production deployment.
func LoadEmbeddedWithManifest(manifestPath string) (*Registry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read deployment manifest %q: %w", manifestPath, err)
	}
	root, err := fs.Sub(embeddedArtifacts, "testdata")
	if err != nil {
		return nil, fmt.Errorf("open embedded deployment artifacts: %w", err)
	...
	if err := fs.WalkDir(root, ".", func(filePath string, entry fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		if entry.IsDir() {
		...
		contents, readErr := fs.ReadFile(root, filePath)
		if readErr != nil {
			return readErr
		}
		files[filePath] = &fstest.MapFile{Data: contents}
		return nil
	}); err != nil {
		return nil, fmt.Errorf("copy embedded deployment artifacts: %w", err)
	}
	files["e2e/deployment.json"] = &fstest.MapFile{Data: body}
	return Load(files)
}
