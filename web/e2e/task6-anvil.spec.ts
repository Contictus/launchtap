...
test.describe("Task 6 Anvil transaction gate", () => {
  ...
  test.beforeAll(() => {
    if (
      ...
      throw new Error("Task 6 Anvil gate requires the real RPC, API, and factory configuration");
  });
  ...
  test("buys and sells through curve controls with a separate approval transaction", async ({
    ...
    await expect(launchStatus).toContainText(/finalized/i, { timeout: 30_000 });
    ...
    await expect
      .poll(
        async () => {
          const response = await page.request.get(`${api}/v1/transactions/${launchHash}`);
          if (!response.ok()) return "";
          const body = (await response.json()) as { events?: { kind: string; token?: string }[] };
          tokenAddress = body.events?.find((event) => event.kind === "token_launch")?.token ?? "";
          return tokenAddress;
        },
        { timeout: 30_000 },
      )
    ...
    await expect(page.getByRole("status").last()).toContainText(/finalized/i, { timeout: 30_000 });
  ...
  test("switches from a wrong chain before signing", async ({ page }) => {
    ...
    await expect(page.getByRole("alert")).toContainText(/Wrong network/i);
  ...
  test("regresses to unavailable after the canonical launch disappears in an Anvil reorg", async ({
    ...
    await expect(status).toContainText(/finalized/i, { timeout: 30_000 });
